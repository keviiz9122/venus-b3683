...
public class AlertManager {
   ...
   public void handleViolation(PlayerData playerData, Check check, Supplier<String> data, double vl, boolean disguise) {
      this.run(() -> {
         this._handleViolation(playerData, check, CachedSupplier.of(data), vl, disguise);
      });
   }
   ...
   private void _handleViolation(PlayerData playerData, Check check, Supplier<String> data, double vl, boolean disguise) {
      if (playerData.isEnabled()) {
         ...
         check.setViolations(check.getViolations() + Math.min(5.0D, vl));
         ...
         if (api != null && api.fireViolationEvent(playerData, check, vl)) {
            ...
            if (config.isBypassEnabled() && !config.isBypassAlerts() && playerData.getPlayer().hasPermission(config.getBypassPermission())) {
               ...
               if (!check.getType().ignore()) {
                  ...
                  if ((double)vl > (double)check.getMaxViolation() * 0.25D && vl % 3 == 0) {
   ...
   public void handleAlertBroadcast(Check check, String message, String playerName) {
      Stream<Player> players = DataManager.getInstance().getPlayers().stream().filter(PlayerData::isAlerts).filter((data) -> {
         return data.isFocused(check);
      ...
      if (config.isAlertClick()) {
         ...
         HoverEvent hoverEvent = new HoverEvent(Action.SHOW_TEXT, TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', config.getAlertClickHover().replace("{player}", playerName))));
         ClickEvent clickEvent = new ClickEvent(net.md_5.bungee.api.chat.ClickEvent.Action.RUN_COMMAND, config.getAlertClickCommand().replace("{player}", playerName));
   ...
   public void handleDebug(PlayerData playerData, Check check, double vl, Supplier<String> data) {
      DataManager.getInstance().getPlayers().stream().filter(PlayerData::isDebug).filter((other) -> {
         return other.getFocus() == null || other.getFocus().equals(check.getType()) && check.getSubType().equals(other.getFocusSubType());
      }).map(PlayerData::getPlayer).forEach((player) -> {
         player.sendMessage(String.format(DEBUG, playerData.getName(), check.getType().getName(), check.getSubType() + (check.getCheckVersion() == CheckVersion.RELEASE ? "" : (check.getCheckVersion() == CheckVersion.EXPERIMENTAL ? "^" : "*")), vl, data.get()));
      });
   }
   ...
   public boolean insertBan(PlayerData playerData, Check check) {
      ...
      if (config.isScrambleBanId()) {
         reason = reason + StringUtil.generateScrambledId(4);
      } else if (check.getType() != CheckType.MANUAL && !config.isRandomBanId()) {
         reason = reason + check.getType().getSuffix() + check.getSubType();
      } else {
         reason = reason + StringUtil.generateRandomId();
      }

      CheckValues values = CheckManager.getInstance().getValues(check);
      List<String> checkCommands = values.hasCommands() ? values.getCommands() : config.getBanCommands();
      List<String> commands = (List)checkCommands.stream().map((command) -> {
         return command.replace("%s", name).replace("{name}", name).replace("{player}", name).replace("{reason}", reason);
      ...
      if (announce) {
         ...
         int var15 = message.length;

         for(int var16 = 0; var16 < var15; ++var16) {
            String line = var14[var16];
            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', line.replace("%s", name).replace("{player}", name).replace("{reason}", reason)));
         }
      }
      ...
      if (config.isBungeeBans()) {
         ...
         NMSManager.getInstance().postToMainThread(() -> {
            commands.forEach((command) -> {
               messagingHandler.handleBan(player, command);
            });
         });
      } else {
   ...
   public void uploadLogs(CommandSender sender, String firstLine, Iterable<Log> logs, boolean admin) {
      ...
      Iterator var6 = logs.iterator();

      while(var6.hasNext()) {
         Log log = (Log)var6.next();
         line.append(log.toString(admin));
      }

      String logUpload = line.toString();
      hastebinService.execute(() -> {
         String[] var4 = HasteBin.ENDPOINTS;
         int var5 = var4.length;
         int var6 = 0;

         while(var6 < var5) {
            String endpoint = var4[var6];
            sender.sendMessage(ChatColor.GRAY + "(Uploading logs to hastebin...)");

            try {
               String logx = HasteBin.paste(logUpload, endpoint);
               sender.sendMessage(VerusPlugin.COLOR + (admin ? "Admin " : "") + "Logs URL: " + ChatColor.WHITE + logx);
               return;
         ...
         try {
            String log = (new PasteBin(new AccountCredentials("oeKULcnP-95AIFqgkDvkgqQU8AGt_0gI", "levansj05", "x4aL_zf#FZn3rT6"))).createPaste(new Paste(firstLine, logUpload, PasteHighLight.TEXT, PasteExpiration.ONE_WEEK, PasteVisibility.UNLISTED));
   ...
   static {
      DEBUG = VerusPlugin.COLOR + ChatColor.BOLD.toString() + "Verus-Debug " + ChatColor.DARK_GRAY + "> " + ChatColor.WHITE + "%s" + ChatColor.GRAY + " failed " + ChatColor.WHITE + "%s Type %s" + ChatColor.GRAY + " VL[" + VerusPlugin.COLOR + "%.1f." + ChatColor.GRAY + "] %s";
