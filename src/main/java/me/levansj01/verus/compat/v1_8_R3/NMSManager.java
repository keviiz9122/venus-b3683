...
import com.ngxdev.viaversion.api.Via;
...
import me.levansj01.verus.compat.api.trove.LongObjectMapWrapper;
...
import org.vspigot.vSpigotConfig;
import org.vspigot.protocolsupport.api.ProtocolSupportAPI;
import org.vspigot.protocolsupport.api.ProtocolVersion;
...
public class NMSManager extends me.levansj01.verus.compat.NMSManager<IBlockData> {
   ...
   private final MutableBlockPosition blockPosition = new MutableBlockPosition();
   private static final Field effects = SafeReflection.access(EntityLiving.class, "effects");
   private static final State<List<IUpdatePlayerListBox>> listState = State.fast(() -> {
      return (List)SafeReflection.getLocalField((Class)MinecraftServer.class, MinecraftServer.getServer(), (String[])("p"));
   });
   ...
   public NMSManager() {
      ...
      try {
         ...
         this.multiblock = this.vspigot = true;
   ...
   public <V> Map<Long, V> long2ObjectHashMap(int initialCapacity, float loadFactor) {
      return (Map)(this.fastUtil ? new Long2ObjectOpenHashMap(initialCapacity, loadFactor) : new LongObjectMapWrapper(new TLongObjectHashMap(initialCapacity, loadFactor)));
   }
   ...
   public IBlockData place(ItemStack stack, Player player, BlockPosition pos, int face, float blockX, float blockY, float blockZ) {
      ...
      if (stack == null) {
         ...
         if (item instanceof ItemBlock) {
            ...
            return block.getPlacedState(entityPlayer.world, new net.minecraft.server.v1_8_R3.BlockPosition(pos.getX(), pos.getY(), pos.getZ()), EnumDirection.fromType1(face), blockX, blockY, blockZ, metadata, entityPlayer);
         } else {
            return null;
         }
      }
   ...
   public Material getType(Player player, World world, IBlockPosition block) {
      ...
      if (worldServer.isLoaded(this.blockPosition)) {
         if (this.vspigot && this.multiblock) {
            MaterialData materialData = (MaterialData)((CraftPlayer)player).getHandle().getFakeBlocks().get(this.blockPosition);
   ...
   public LazyData toLazy(IBlockData data) {
      return this.toLazy(data, State.fast(() -> {
         return net.minecraft.server.v1_8_R3.Block.d.b(data);
      }));
   }

   public LazyData toLazy(IBlockData data, State<Integer> id) {
      if (data == null) {
         return LazyData.AIR;
      } else {
   ...
   public Cuboid getBoundingBox(Player player, World world, IBlockPosition block) {
      ...
      if (!worldServer.isLoaded(this.blockPosition)) {
         ...
         if (this.vspigot && this.multiblock) {
            blockData = worldServer.getType(((CraftPlayer)player).getHandle(), this.blockPosition);
         } else {
            blockData = worldServer.getType(this.blockPosition);
         }
   ...
   public ClientVersion getVersion(Player player) {
      if (!this.viaversionplugin && !this.nspigot) {
         ...
         } else if (this.protocolsupportplugin) {
   ...
   public void sendTeleport(Player player, ILocation loc) {
      ((CraftPlayer)player).getHandle().playerConnection.sendPacket(new PacketPlayOutPosition(loc.getX(), loc.getY(), loc.getZ(), loc.getYaw(), loc.getPitch(), Collections.emptySet()));
   }
   ...
   public void sendBlockUpdate(Player player, World world, int x, int y, int z) {
      ...
      if (worldServer.isLoaded(this.blockPosition)) {
         if (this.vspigot) {
            ...
            ((CraftPlayer)player).getHandle().playerConnection.sendPacket(new PacketPlayOutBlockChange(this.blockPosition, blockData));
         } else {
